...
| `src/statusBar.ts` | ステータスバーの描画（サマリ + クォータ + セッションごとのカードを動的に生成） |
...
## 2. ステータスバー（サマリ + クォータ + セッションごとのカード）

すべて右寄せで常駐し、クリックでダッシュボードを開く。グローバルな2アイテム
（サマリ・クォータ）に加え、**アクティブなセッションごとに1枚のカード**が動的に増減する。

### 2.1 サマリインジケータ（`$(pulse) $N.NN`）

- マシン全体の真にグローバルな指標として**今日の総コスト**を主表示。
- ホバーで Markdown テーブルのツールチップ:
  今日 / 現在セッションの「コスト・入出力トークン・キャッシュ作成/読取・メッセージ数」。
- ログ未検出時は「データなし」、エラー時は警告色で縮退表示（このときセッションカードは全て撤去）。
...
### 2.3 セッションカード（1セッション = 1カード、`$(pulse) プロジェクト名 モデル N% $(zap)M:SS`）

複数の Claude Code セッションを並行実行しているとき、どのセッションのモデル/コンテキスト/
キャッシュかを取り違えないよう、**指標をセッション単位の1枚のカードに統合**する。
カードはセッションごとに生成され、アイドルになると消える（数が起動中セッション数に追従）。

- **対象**: 最終リクエストが `sessionCardRecencyMinutes`（既定60分）以内のセッション。
  最新順に最大 `maxSessionCards`（既定5枚）まで表示。
- **表示内容**（すべてそのセッション固有）:
  - **モデル名**（そのセッションの最新リクエストのモデル）
  - **コンテキスト充填率** `N%`（`enableContextHealth` 有効時のみ。状態 `rot` で警告アイコン + 警告色背景）
  - **プロンプトキャッシュ残り** `$(zap)M:SS`（そのセッションの最終リクエストからの5分TTLカウントダウン、
    1秒間隔で独立更新。期限切れは `$(zap)—`、残り1分未満で警告色）
- **ホバーのツールチップ**:
  - ヘッダ（プロジェクト名・モデル・セッションID先頭8桁）
  - **そのセッションの最初のプロンプト（100字以内）** — 「どのセッションか」をひと目で判別する手がかり。
    `isMeta`（ハーネス注入）行と `/clear` 等のスラッシュコマンド展開はスキップして、最初に
    人間が打った実プロンプトを抽出（セッションID単位でキャッシュ）。
  - このセッションのキャッシュ状態（warm/cold と残り時間）
  - `enableContextHealth` 有効時は Context Health の詳細（窓サイズ、成長スパークライン、
    成長ペースと上限到達 ETA、構成バー、トピック別内訳、検出シグナル一覧、`/clear` 推奨等）
- 設定 `contextHealthRotNotification`（既定 off）を有効にすると、各セッションが初めて
  `rot` になった時に**1回だけトースト通知**（プロジェクト名付き、同一セッションは30分の沈黙後に再武装、
  セッションごとに独立管理）。

> 注: `/exit` や `/clear` は会話ログに終了マーカーを残さないため、拡張側からは「書き込みが
> 止まっただけ」のアイドルと区別できない。よってカードの撤去は時間ベース
> （`sessionCardRecencyMinutes`）のみで判定する。
...
| `contextHealthRotNotification` | false | `rot` 初回検出時の1回限りトースト通知（オプトイン、セッションごと） |
| `sessionCardRecencyMinutes` | 60 | この分数以内に書き込みのあったセッションをカード表示（古いものは撤去） |
| `maxSessionCards` | 5 | ステータスバーに同時表示するセッションカードの最大数（最新順） |
...
- **アイドル認識リフレッシュ**: 全 `.jsonl` の最新 mtime を見て、前回ロード以降に変更が
  なければ重い再計算をスキップ（クォータの更新と、各セッションカードのキャッシュ残り
  カウントダウンの独立更新のみ）。
- **セッションカードの軽量更新**: カードのキャッシュ残りは1秒間隔でテキストのみ再描画し、
  重いツールチップ（Markdown）の再構築は全リフレッシュ時のみ。最初のプロンプトは
  セッションID単位でキャッシュし `.jsonl` の再読み込みを避ける。
