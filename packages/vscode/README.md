...
| インジケータ | 表示内容 |
|---|---|
| `$(pulse)` 使用量 | 今日のコスト / アクティブセッションのコスト |
| `$(dashboard) 5h:N% wk:N%` クォータ | 実際の 5時間・週次クォータ利用率（80%以上で黄・95%以上で赤） |
| `$(zap) M:SS` キャッシュ余熱 | プロンプトキャッシュ 5分 TTL のカウントダウン |
| `$(book) ▰▰▰▱▱ N%` Context Health | アクティブセッションのコンテキスト充填率・劣化状態 |
...
| `contextHealthRotNotification` | false | Context rot 検出時のトースト通知（オプトイン） |
