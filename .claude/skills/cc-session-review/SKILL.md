...
### 4b. 失敗モード分類(MAST taxonomy)

判定が partial / abandoned の場合、失敗を **MAST**(Multi-Agent System Failure Taxonomy,
arXiv:2503.13657。人手アノテーションと κ=0.88 で一致する実証ベースの分類)に当てはめる。
複数該当可。単一エージェントの Claude Code セッションでは「エージェント間」をユーザー⇔
Claude およびメイン⇔サブエージェントの関係に読み替える。該当証拠(プロンプト見出し・
エラー・反復呼び出し)を必ず添え、証拠がないモードは挙げない。

| カテゴリ | モード | セッションでの現れ方の例 |
|----------|--------|--------------------------|
| **1. 仕様の問題** | 1.1 タスク仕様への不服従 | 指示と違う成果物・無視された制約 |
| | 1.2 役割仕様への不服従 | 提案だけ頼んだのに実装まで進めた等 |
| | 1.3 ステップの反復 | `repeatedCalls` の高反復(同一編集・同一コマンド) |
| | 1.4 会話履歴の喪失 | コンパクション後の文脈取り違え・既決事項の再質問 |
| | 1.5 終了条件の不認識 | 完了後も作業を続ける・止めどころを誤る |
| **2. 連携の不整合** | 2.1 会話のリセット | 突然の方針巻き戻し |
| | 2.2 明確化を求めない | 曖昧な指示のまま誤った方向に進む |
| | 2.3 タスクの脱線 | 頼んでいない作業への逸脱 |
| | 2.4 情報の隠蔽 | エラー・スキップを報告しない |
| | 2.5 入力の無視 | ユーザーの訂正が次ターンに反映されない |
| | 2.6 推論と行動の不一致 | 言っていることと実行コマンドが食い違う |
| **3. 検証の不備** | 3.1 早すぎる終了 | テスト未実行のまま完了報告 |
| | 3.2 検証の欠如・不完全 | コンパイルのみでテストなし等 |
| | 3.3 誤った検証 | 緑になったテストが対象を検証していない |

分類はユーザー自身が直せる軸(仕様の書き方 / 連携=指示・確認の仕方 / 検証の頼み方)に
紐づくため、§6 の提案はこのカテゴリに対応させること。レビューを蓄積すると
「自分の失敗はどのカテゴリに偏るか」が見えてくる(レポート末尾に該当カテゴリを明記)。
...
- 構成:
  ...
  4. **エラーと迷走**: エラーの種類別整理(原因の意味分類)、ループ箇所、
     該当する MAST 失敗モード(§4b。partial/abandoned のときのみ)
