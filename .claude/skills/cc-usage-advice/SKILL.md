...
| コンテキスト運用 | `contextHealth`(fillRatio, signals, baselineTokens, reclaimableTokens, largestUserPromptTokens, composition の `injectedContext`) | largeBaseline → CLAUDE.md 等の肥大化指摘 / reclaimable 大 → ツール出力の絞り込み(offset/limit 付き Read など) / largeUserPrompt シグナル → 巨大な貼り付け・重いスキル本文の指摘 / injectedContext が組成上位 → 重いスキルの見直し |
| 滞留コスト | `contextHealth.maskingSavingsUSD`(古いツール出力を 10 ターン超で マスクしていた場合の節約額。全ターン累積) | 額が大きければ「古い出力の単純な切り捨てで LLM 要約と同等の効果がコスト半分以下で得られる(arXiv:2508.21433)」を根拠に、**コンパクション(LLM 要約)より早めの /clear・新セッション・ハンドオフを優先**するよう助言 |
| キャッシュバスト原因 | `contextHealth.cacheBusts[]`(at, cause: ttlExpiry/modelSwitch/parallel/other, wastedTokens)、`cacheWastePct` / `sessionCostUSD` | ttlExpiry → 離席をまたぐセッション継続をやめる / modelSwitch → セッション途中の /model を避ける / parallel → 同時多発リクエストの初回書込重複(ユーザー側対策は限定的、事実として報告)。waste 比率は「戦略的キャッシュ管理で 41–80% 削減が達成可能(arXiv:2601.06007)」を基準線に評価 |
| トークン経済 | `contextHealth.inputOutputRatio`(入力:出力比)、`wasteTokens`(キャッシュ再書込+失敗ツール) | エージェント作業の実測レンジは 2:1〜150:1(arXiv:2605.09104)。100:1 超なら「コンテキストの持ち回りが支配的」— 滞留コスト・組成の指摘と束ねて提示 |
| 行き詰まり | `signals` の `stuckSession`(直近ツールエラー率の急騰) | 出ていれば「タスクの仕切り直し・指示の具体化・新セッション」を提案(継続ランの早期打ち切りはコスト効率が高い: arXiv:2601.05777) |
| ツール・エラー | `activity.tools` のエラー率上位、`toolErrors`/`totalToolCalls` | エラー率が高いツールの使い方改善 |
| 思考・出力配分 | `thinkingTokensEst` vs `assistantTextTokensEst`、`mainOutputTokens` vs `sidechainOutputTokens` | サブエージェント比率が低く重作業が多ければ並列委譲を提案。`thinkingTokensEst` が 0 でも `thinkingBlocksSeen` > 0 なら「思考テキストは非保存(不明)」であり 0% と書かない |
| クォータ圧力 | quota-history の推移(時間帯ごとの増分、80%超の頻度) | 逼迫時間帯の把握、重いタスクをリセット直後に回す等の運用提案 |
| セッション習慣 | `sessions` の peakContextTokens 分布、1セッションの長さ | 200K 近くまで使い切る癖があれば早めの /clear・ハンドオフを提案。長コンテキストでは品質劣化が実測されている(長尺末尾では重要情報の見逃しが 2〜30 倍: arXiv:2605.12366)ため、長尺継続が避けられない場合は**節目ごとにゴール・制約を再掲する(periodic reminder)**運用を提案 |
