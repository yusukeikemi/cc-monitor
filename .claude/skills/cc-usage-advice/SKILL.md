...
| コンテキスト運用 | `contextHealth`(fillRatio, signals, baselineTokens, reclaimableTokens, largestUserPromptTokens) | largeBaseline → CLAUDE.md 等の肥大化指摘 / reclaimable 大 → ツール出力の絞り込み(offset/limit 付き Read など) / largeUserPrompt シグナル → 巨大な貼り付け・重いスキル本文の指摘 |
| キャッシュバスト原因 | `contextHealth.cacheBusts[]`(at, cause: ttlExpiry/modelSwitch/parallel/other, wastedTokens) | ttlExpiry → 離席をまたぐセッション継続をやめる / modelSwitch → セッション途中の /model を避ける / parallel → 同時多発リクエストの初回書込重複(ユーザー側対策は限定的、事実として報告) |
